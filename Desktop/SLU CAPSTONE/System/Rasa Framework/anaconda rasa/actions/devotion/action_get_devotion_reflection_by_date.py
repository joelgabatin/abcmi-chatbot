...
class ActionGetDevotionReflectionByDate(Action):
    def name(self) -> Text:
        return "action_get_devotion_reflection_by_date"

    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        date_entity = next(tracker.get_latest_entity_values("devotion_date"), None)
        if not date_entity:
            dispatcher.utter_message(text="Please specify a date. For example: 'What is the September 6, 2026 devotion/reflection?'")
            return []
        try:
        ...
        if devotion:
            dispatcher.utter_message(
                text=(
                    f"Devotion for {devotion['date']} — \"{devotion['title']}\"\n\n"
