...
class ActionGetEventTime(Action):

    def name(self) -> Text:
        return "action_get_event_time"
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        if not event_name:
            dispatcher.utter_message(
                text="Could you please tell me the title of the event you're asking about?"
            )
            return []
        ...
        if events:
            e = events[0]
            time_str = e.get("time") or "Time not set"
            dispatcher.utter_message(
                text=f"{e['title']} is scheduled at {time_str}."
            )
        else:
            dispatcher.utter_message(
